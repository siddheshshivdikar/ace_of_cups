...
const multiInput = ["9C 9D 8D 7C 3C 2S KD TH 9H 8H",
...
var testInput = multiInput.slice(0, 4)

function generateListItems(arg){
  let items = "";
  for(let i = 0; i < arg.length; i++){
    items += `<li class="input-list-item">${arg[i]}</li>`;
  }
  return items;
}

document.querySelector("main").innerHTML = `
<div class="input-list row" id="input-list">
</div>
`;
// ${generateListItems(testInput)}

// const inputHands = [(input).slice(0, 14),  (input).slice(-14)]
...
function card_value(card) {
  if (card.charAt(0) == 'A') {
    ...
    function playGame(playerHands) {

        var node = document.createElement("div")
        // document.createElement("div").setAttribute("class", "card-content white-text");
        // document.createElement("span").setAttribute("class", "card-title")
        
        playerHands.map(function (hand, index) {
            // console.log(hand)
        ...
        node.innerHTML = `<div class="col s12 m6 l4 xl3"><div class="card blue-grey darken-1" style="min-height: 270px"><div class="card-content white-text">
                                <span class="card-title">
                                    ${playerOneRank > playerTwoRank ? "Player One Wins" : playerOneRank < playerTwoRank ? "Player Two Wins" : "Tie"}
                                </span>
                                <p>Hands: ${playerHands}</p>
                            </div>
                            <div class="card-action">
                                <p class="white-text">Player One: ${playerOneRank === 24 ? "Royal Flush" : playerOneRank === 23 ? "Straight Flush" : playerOneRank === 22 ? "Four of a Kind" : playerOneRank === 21 ? "Full House" : playerOneRank === 20 ? "Flush" : playerOneRank === 19 ? "Straight" : playerOneRank === 18 ? "Three of a Kind" : playerOneRank === 17 ? "Two Pair" : playerOneRank === 16 ? "Pair" : playerOneRank === 15 ? "High Card" : `Highest Card: ${playerOneRank}`}</p>
                                <p class="white-text">Player Two: ${playerTwoRank === 24 ? "Royal Flush" : playerTwoRank === 23 ? "Straight Flush" : playerTwoRank === 22 ? "Four of a Kind" : playerTwoRank === 21 ? "Full House" : playerTwoRank === 20 ? "Flush" : playerTwoRank === 19 ? "Straight" : playerTwoRank === 18 ? "Three of a Kind" : playerTwoRank === 17 ? "Two Pair" : playerTwoRank === 16 ? "Pair" : playerTwoRank === 15 ? "High Card" : `Highest Card: ${playerTwoRank}`}</p>
                            </div>
                            </div>
                            </div>`
        document.getElementById("input-list").appendChild(node);
...
function check_cases(arg){
        for(let i = 0; i < arg.length; i++){
        const inputHands = [(arg[i]).slice(0, 14), (arg[i]).slice(-14)]
        playGame(inputHands)
      }
    ;
}

check_cases(multiInput)
